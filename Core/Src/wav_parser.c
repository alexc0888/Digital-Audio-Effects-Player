...
int parseWavHeader(wav_header_t *wavHeader)
{
	...
	if(readWavHeader(rdBuff, HEADER_BUFF_SIZE) != PARSE_SUCCESS)
	{
	...
	case SYS_BIG_ENDIAN:
		...
		// Note: All the .WAV data from here on out is little-endian, so be sure to perform a conversion for every sample if on big endian system
		break;
	}
...
int skipListSubChunk(wav_header_t *wavHeader)
{
	...
	uint32_t listSubchunk2Buff[bytesToSkip / sizeof(uint32_t)];
	if(readWavHeader(listSubchunk2Buff, bytesToSkip / sizeof(uint32_t)) != PARSE_SUCCESS)
	{
	...
	if(readWavHeader(dataSubchunk2Buff, 2) != PARSE_SUCCESS)
	{
...
int readWavHeader(uint32_t *rdBuff, uint32_t arrSize)
{
	// grab 44 byte header from storage platform (SD card in this case)
	if(sdReadFile((void *)rdBuff, arrSize * sizeof(uint32_t)) != SD_SUCCESS)
	{
		return PARSE_FAIL;
	}
